...
<title>Обработка на резултата с mysql_fetch_array() с MYSQL_BOTH</title>
</head>
...
while ($row = mysql_fetch_array($result, MYSQL_BOTH)) {
	printf("<br>");
    printf("Поръчка ID: %s  Поръчка дата: %s  Клиент ID: %s  Общо: %s"   , $row[0], $row[1], $row["CustomerID"], $row["SubTotal"]);
}
